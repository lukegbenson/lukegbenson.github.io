...
We will not employ any complex machine learning approach here. Instead, we'll engineer a handful of features from city parking lot footprints and fit a simple linear model. Let's see what we get.
...
Rather than pulling in outside data to fill in these gaps, we'll construct a few variables using only the geographic footprint of the lots. The goal is not to build the best possible model of car usage. Instead, we want to see how much explanatory power we can get from the small amount of information we do have.
...
At this point, there are several ways we could make this more complicated. We could add a set of demographic, geographic, and economic variables. We could use a random forest model, include a cross-validation scheme, or just generally use more sophisticated modeling techniques.

But we're not going to do that.

The purpose here is that we want to estimate a multifaceted, complex behavioral data point using an almost comically small amount of information. We keep the model appropriately simple: a linear regression using the four parking-lot variables above.
